...
-- The updates column may be either JSONB or TEXT depending on migration state
-- This fix handles both cases by explicitly casting
...
BEGIN
    ...
    ELSIF v_item->>'operation' = 'update' AND v_item->>'external_id' IS NOT NULL THEN
      ...
      SET
        ...
        embedding = CASE WHEN v_item->'embedding' IS NOT NULL AND v_item->'embedding' != 'null'::jsonb
          ...
          ELSE embedding
        END,
        updates = (v_item->>'updates_json')::jsonb
  ...
  GET DIAGNOSTICS v_audit_logs_created = v_audit_logs_created + ROW_COUNT;
